...
from refresh_token.refresh_token_gcp_Sammi4 import (
    get_latest_ac_token_gcp4,
    request_new_ac_token_refresh_token_gcp4,
)
...
def check_if_need_update_token(**context):
    ...
    if (
        last_ac_time + 3000 < current_timestamp
    ):  # means token has been expired, default is 3600, make it 3000 to ensure we have enough time
...
def get_artist_uris():
    ...
    credentials = service_account.Credentials.from_service_account_file(path_credential)
...
def get_artist_data(**context):
    ...
    access_token = context["task_instance"].xcom_pull(
        task_ids="check_if_need_update_token", key="access_token"
    )
...
with DAG(
    "spotify_ArtistUri_Sammi.py",
    default_args=default_args,
    schedule_interval="@monthly",
    catchup=False,
) as dag:
    check_if_need_update_token = PythonOperator(
        task_id="check_if_need_update_token", python_callable=check_if_need_update_token
    )
