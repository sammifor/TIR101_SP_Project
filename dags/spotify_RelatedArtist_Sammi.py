...
# Related_Artist_API
def related_artist_data(**context):
    ...
    for artist_uri in artist_uris:
        ...
        #測試用
        # if len(artistRelated_list) > 10:
        #     break

        artistRelated_list.append(artist_data)

        n = random.randint(1,3) ## gen 1~3s
        time.sleep(n)

    with open('/opt/airflow/dags/rawdata/related_artist.json','w') as f:
        json.dump(artistRelated_list,f)
    

def store_data_in_gcs():
    
    with open('/opt/airflow/dags/rawdata/related_artist.json','r') as f:
        artistRelated_list = json.load(f)
    
    #Extend data
    df_artistRelatedArtist = pd.json_normalize(artistRelated_list)
    df_artistRelated = df_artistRelatedArtist.explode('artists')
    df_artistRelatedExplode = pd.json_normalize(df_artistRelated['artists']).drop(columns='images')
    df_artistRelatedGenre = df_artistRelatedExplode.explode('genres')

    #Upload to GCS
    local_file_path = 'artistRelatedArtist17_21.csv'
    gcs_bucket = 'api_spotify_artists_tracks'
    gcs_file_name = f'output/{local_file_path}'

    df_artistRelatedGenre.to_csv(local_file_path, index=False)
...
with DAG('spotify_RelatedArtist_Sammi.py',
    ...
    related_artist_data = PythonOperator(
        task_id='related_artist_data',
        python_callable=related_artist_data,
...
check_if_need_update_token >> related_artist_data >> store_data_in_gcs
